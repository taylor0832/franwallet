...
function derive(location,life){
 ...
 const reviewed=lifecycle.some(r=>r.kind==='readiness_evidence'&&r.data?.status==='attached'&&r.data.document_ids?.some(id=>ids.has(id)))
  ||lifecycle.some(r=>(r.kind==='compliance'||(r.kind==='entity'&&r.data?.name&&!r.data?.onboarding_draft)))
...
function render(){
 ...
 guide.innerHTML=`<div class="activation-top"><strong>${state.complete?escape(context.wallet.name)+' foundation saved':'Set up your wallet'} <span>${state.count} / 4</span></strong><span>${state.complete?'Your first sources and key details are saved. Open your dashboard to review periods and next actions.':'Your progress is saved. Pick up here anytime.'}</span>${state.complete?'<button type="button" class="btn btn-ghost" id="activation-add-location">＋ Add a new location</button>':`<button type="button" class="btn btn-primary" data-activation-step="${state.index}">${buttons[state.index]} →</button>`}</div>${state.complete?'<details class="activation-finished"><summary>Review completed setup</summary>':''}<ol class="activation-steps">${labels.map((label,i)=>`<li class="${state.done[i]?'done':state.index===i?'current':''}"><button type="button" data-activation-step="${i}" ${state.index===i?'aria-current="step"':''}><span aria-hidden="true">${state.done[i]?'✓':i+1}</span>${label}</button></li>`).join('')}</ol>${state.complete?'</details>':''}`;
